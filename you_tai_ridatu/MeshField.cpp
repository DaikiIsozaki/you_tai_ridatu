...
void MeshField_Init(float meshW, int meshXnum, int meshZnum)
{
...
void MeshCylinder_Init(float meshH, float radius, int meshXnum, int meshYnum)
{
	...
	for (int y = 0; y < VertexY; y++)
	{
		if (y == 0)
		{
			for (int x = 0; x < VertexX; x++)
			{
				...
				g_Mfield[n].color = D3DCOLOR_ARGB(0, 0, 0, 255);
				g_Mfield[n].uv = D3DXVECTOR2(x * (1.0f / meshXnum), y * (1.0f / meshYnum));
			}
		}
		else
		{
			...
			for (int x = 0; x < VertexX; x++)
			{
				...
				g_Mfield[n].normal = D3DXVECTOR3(0.0, 1.0, 0.0);
				g_Mfield[n].color = D3DCOLOR_ARGB(0, 255 / VertexY * y, 255 / VertexY * y, 255);
				g_Mfield[n].uv = D3DXVECTOR2(x * (1.0f / meshXnum), y * (1.0f / meshYnum));
			}
		}
...
void MeshField_Draw(int index)
{
	...
	if (index == 0)
	{
		pD3DDevice->SetTexture(0, Texture_GetTexture(10));
	}
	if (index == 1)
	{
		pD3DDevice->SetTexture(0, Texture_GetTexture(27));
	}
	if (index == 2)
	{
		pD3DDevice->SetTexture(0, Texture_GetTexture(28));
	}
